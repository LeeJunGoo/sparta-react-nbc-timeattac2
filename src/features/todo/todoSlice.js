...
import { createTodo, deleteTodo, getSingleTodo, getTodos, updateTodo } from "../../api/todo-api";

export const getSingleTodoThunk = createAsyncThunk("todo/getSingleTodos", getSingleTodo);
...
export const toggleTodoThunk = createAsyncThunk("todo/updateTodo", async (id, { getState }) => {
...
export const todoSlice = createSlice({
  ...
  reducers: {
    sortTodos: (state, action) => {
      ...
      if (sortOrder === "asc") {
        state.todos = state.todos.sort((a, b) => new Date(a.deadline) - new Date(b.deadline));
        return;
      }
      state.todos = state.todos.sort((a, b) => new Date(b.deadline) - new Date(a.deadline));
    },
  },
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteTodoThunk.fulfilled, (state, action) => {
      const targetIndex = state.todos.findIndex((todo) => todo.id === action.payload);
    ...
    builder.addCase(getSingleTodoThunk.fulfilled, (state, action) => {
